  ...
  async initializeGateways() {
    try {
      ...
      for (const gateway of activeGateways) {
        switch (gateway.name) {
          case 'stripe':
            if (gateway.configuration && gateway.configuration.secretKey) {
              this.gateways.set('stripe', new Stripe(gateway.configuration.secretKey, {
                apiVersion: '2023-10-16'
              }));
            } else {
              console.warn('Stripe gateway is active but secretKey is missing from configuration');
            }
            break;
          case 'razorpay':
            if (gateway.configuration && gateway.configuration.publicKey && gateway.configuration.secretKey) {
              this.gateways.set('razorpay', new Razorpay({
                key_id: gateway.configuration.publicKey,
                key_secret: gateway.configuration.secretKey
              }));
            } else {
              console.warn('Razorpay gateway is active but publicKey or secretKey is missing from configuration');
            }
            break;
          case 'paypal':
            if (gateway.configuration && gateway.configuration.publicKey && gateway.configuration.secretKey) {
              // PayPal initialization logic would go here if using a SDK
              console.info('PayPal gateway configured successfully');
            } else {
              console.warn('PayPal gateway is active but publicKey or secretKey is missing from configuration');
            }
            break;
        }
      }
